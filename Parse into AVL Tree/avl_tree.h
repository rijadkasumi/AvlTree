...
// int tree_Path() 	      --> Checks the path of the length of the tree
...
private:
	...
	int total_Number_Rec = 0; // total number of recurisve calls
	int remove_Calls = 0;    // total number of remove calls
	...
	void remove( const Comparable & x, AvlNode * & t )
	{
		...
		if( x < t->element ){
			++remove_Calls; //pre increment  remove_Calls from Tree
			remove( x, t->left );}
		else if( t->element < x ){
			++remove_Calls; 
			remove( x, t->right );}
    ...
    void findF(const Comparable& x, AvlNode *t) const {
	 ...
	 bool findF2( const Comparable & x, Comparable & y, AvlNode *t){
        ...
        if( t == nullptr ){
            ...
            ++total_Number_Rec;; // number of recursion calls
    ...
    float Depth_Of_Tree(AvlNode * t, float depth) const {
        if (t == nullptr) {
            ...
            return Depth_Of_Tree(t->left, depth + 1) + Depth_Of_Tree(t->right, depth + 1) + depth; //height of left subtree +1 + depth of righ subtree +1 + depth
        }
    }
